...
//    public static String getBlockRegistryName(ServerLevel serverLevel, Player player){
//        return serverLevel.getBlockState(getPlayerPOVHitResult(player.level(), player, ClipContext.Fluid.SOURCE_ONLY).getBlockPos()).getBlock().builtInRegistryHolder().key().location().getNamespace() +
//               ":" +
//               serverLevel.getBlockState(getPlayerPOVHitResult(player.level(), player, ClipContext.Fluid.SOURCE_ONLY).getBlockPos()).getBlock().builtInRegistryHolder().key().location().getPath();
//    }
